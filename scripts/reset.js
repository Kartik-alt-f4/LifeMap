...
// Run from api/ folder (same pattern as embed-seed):
//   cd api && node ../scripts/reset.js [--hard]
//   cd api && node ../scripts/reset.js --hard   ← also clears skill XP + stat values
//
// Soft reset keeps: skill/stat definitions, shop items, config
...
async function run() {
  ...
  console.log('  Clears: tasks, templates, ledgers, sessions, snapshots, player XP/gold/streak.\n')
  ...
  // ORDER MATTERS — clear ledgers before tasks (FK constraints)
  ...
  // Reset player row (id=1 always exists)
  ...
  // daily_state: day_streak, mandatory_met, day_off_granted, date
  ...
  if (HARD) {
    // task_skill and task_stat have no id column — delete by task_id existence
